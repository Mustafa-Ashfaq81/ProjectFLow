...
Future<void> acceptreq(
    BuildContext context, Map<String, dynamic> task, String username) async {
  ...
  try {
    ...
    updatedtask['collaborators'] =
        colabz; // add collaborator(this user who accepted request) to sender's task
    var snapshot =
    ...
    snapshot = await userCollection
        ...
        .get(); //add that task for user who accepted the colab request
    ...
    await updaterooms(
        task, username); //update group chats and requests in cache
...
Future<void> updateColabinDatabase(
    String username, Map<String, dynamic> task) async {
  try {
    await FirebaseFirestore.instance
      ...
      for (var doc in querySnapshot.docs) {
        doc.reference.update({
          ...
          doc.reference.get().then(
              (docSnapshot) // Check if 'usernames' is now empty and delete the record
              {
